...
class TempMail {
  ...
  async createAccount(namePrefix = 'user') {
    ...
    try {
      const res = await axios.post(`${API_BASE}/accounts`, { address, password });
  ...
  async login() {
    try {
      const res = await axios.post(`${API_BASE}/token`, { address: this.email, password: this.password });
  ...
  async getMessages() {
    ...
    try {
      const res = await axios.get(`${API_BASE}/messages`, { headers: { Authorization: `Bearer ${this.token}` } });
      return res.data['hydra:member'];
    } catch (err) {
      if (err.response?.status === 401) { await this.login(); return this.getMessages(); }
  ...
  async getMessage(messageId) {
    if (!this.token) throw new Error('未登录');
    try {
      const res = await axios.get(`${API_BASE}/messages/${messageId}`, { headers: { Authorization: `Bearer ${this.token}` } });
      return res.data;
    } catch (err) {
      if (err.response?.status === 401) { await this.login(); return this.getMessage(messageId); }
      ...
      try {
        ...
        if (messages.length > lastMsgCount) {
          ...
          for (const msg of messages) {
            ...
            if (code) { console.log(`✅ 提取到验证码: ${code}`); return code; }
          }
  ...
  extractCode(message) {
    const text = [message.text || '', message.html?.[0] || message.html || '', message.subject || ''].join('\n');
  ...
  async cleanup() {
    ...
    try {
      await axios.delete(`${API_BASE}/accounts/${this.accountId}`, { headers: { Authorization: `Bearer ${this.token}` } });
